...
% ----------------
function results = clustered_analysis_with_fooof_and_moving_window(data, cluster, amount_of_nodes_per_cluster, srate, window_size, varargin)

    % check if inputs are given and display the help otherwise
    % input check: if no arguments are entered, print the help and stop
    if nargin < 5
        help clustered_analysis_with_fooof_and_moving_window
        return
    end
    ...
    cellRequirements = @(x) (isa(x, 'cell')) && ~isempty(x);
    requirements = @(x) (isa(x, 'double')) && ~isempty(x);
        
    addRequired(p, 'data', dataRequirements);
    addRequired(p, 'cluster', cellRequirements);
    addRequired(p, 'amount_of_nodes_per_cluster', requirements);
    addRequired(p, 'srate', requirements);
    addRequired(p, 'window_size', requirements);
    addOptional(p, 'window_steps', 100 ,requirements);
    
    % parse the input
    parse(p, data, cluster, amount_of_nodes_per_cluster, srate, window_size, varargin{:});
    
    data = p.Results.data;
    eeg_data = data.data;
    events = data.events;
    cluster = p.Results.cluster;
    amount_of_nodes_per_cluster = p.Results.amount_of_nodes_per_cluster;
    ...
    if length(cluster) ~= length(amount_of_nodes_per_cluster)
        error('Cluster length and amount_of_nodes_per_cluster length are not equal.')
    end
    
    if size(eeg_data,1) ~= sum(amount_of_nodes_per_cluster)
        error(['Total amount of nodes per cluster (here ' num2str(size(data,1)) ') must correspond to amount of channel(' num2str(size(eeg_data,1)) ').'])
    end
